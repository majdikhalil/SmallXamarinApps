...
namespace XamarinInsightsAndroid
{
    [Activity(Label = "XamarinInsightsAndroid", MainLauncher = true, Icon = "@drawable/icon")]
    public class MainActivity : Activity
    {
        public WorkCompletedEvent WorkCompleted { get; set; }

        Button btnSingle, btnMultiple, btnDivZero, btnFileEx, btnNullRef, btnPartial, btnTimer, btnMulti, btnCrash;

        protected override void OnCreate(Bundle bundle)
        {
            base.OnCreate(bundle);
            SetContentView(Resource.Layout.Main);
            // before we start, let's set the UI bits

            btnSingle = FindViewById<Button>(Resource.Id.btnSingleId);
            btnMultiple = FindViewById<Button>(Resource.Id.btnMultiId);
            btnDivZero = FindViewById<Button>(Resource.Id.btnDivZero);
            btnFileEx = FindViewById<Button>(Resource.Id.btnFileEx);
            btnNullRef = FindViewById<Button>(Resource.Id.btnNRE);
            btnPartial = FindViewById<Button>(Resource.Id.btnPartial);
            btnTimer = FindViewById<Button>(Resource.Id.btnTimer);
            btnMulti = FindViewById<Button>(Resource.Id.btnMultiTrack);
            btnCrash = FindViewById<Button>(Resource.Id.btnUncaught);

            // until an ID is sent, disable all the buttons
            btnDivZero.Enabled = btnFileEx.Enabled = btnNullRef.Enabled = btnPartial.Enabled = btnTimer.Enabled = btnMulti.Enabled = false;

            // fire the insights method
            StartInsights();
        }

        private void StartInsights()
        {
            WorkCompleted = new WorkCompletedEvent();

            // initialise registration - needs an API key and context
            // 1. Android
			Insights.Initialize(Constants.InsightsApiKey, ApplicationContext);
            ...
            btnSingle.Click += delegate
            {
                ...
                btnDivZero.Enabled = btnFileEx.Enabled = btnNullRef.Enabled = btnPartial.Enabled = btnTimer.Enabled = btnMulti.Enabled = true;
